...
namespace Argent::Graphics
{
	...
	void ArGraphics::DrawDebug()
	{
#ifdef _DEBUG
#endif
	}
		
	HRESULT ArGraphics::LoadTexture(std::string filename, ID3D12Resource** resource)
	{
		DirectX::TexMetadata metaData{};
		DirectX::ScratchImage scratchImage{};
		
		HRESULT hr{ S_OK };
		hr = DirectX::LoadFromWICFile(Helper::String::GetWideStringFromString(filename).c_str(),
		...
		
		{
			...
			Microsoft::WRL::ComPtr<ID3D12Resource> uploadBuffer;
			{
				...
				resDesc.Width = Helper::Math::CalcAlignmentSize(rowImage->rowPitch ) * rowImage->height;
				resDesc.Height = 1;
			...
			//テクスチャのマップとコピー
			{
				...
				UINT rowPitch = static_cast<UINT>(Helper::Math::CalcAlignmentSize(rowImage->rowPitch));
				...
				src.PlacedFootprint.Footprint.RowPitch = static_cast<UINT>(Helper::Math::CalcAlignmentSize(rowImage->rowPitch));
				...
				resourceCmdBundle->cmdList->CopyTextureRegion(&dst, 0, 0, 0, &src, nullptr);
				//mResourceCmdList->CopyTextureRegion(&dst, 0, 0, 0, &src, nullptr);
				...
				resourceCmdBundle->cmdList->ResourceBarrier(1, &barrier);
				resourceCmdBundle->cmdList->Close();
				/*mResourceCmdList->ResourceBarrier(1, &barrier);
					mResourceCmdList->Close();*/
		
		
				ID3D12CommandList* cmdlists[]{ resourceCmdBundle->cmdList.Get() };

				resourceQueue->cmdQueue->ExecuteCommandLists(1, cmdlists);
				resourceQueue->SetFence(1);
			}
		}
		
		resourceCmdBundle->cmdAlloc.Get()->Reset();
		resourceCmdBundle->cmdList.Get()->Reset(resourceCmdBundle->cmdAlloc.Get(), nullptr);
		return hr;
	}
		
	HRESULT ArGraphics::LoadTexture(std::wstring filename, ID3D12Resource** resource)
	{
		DirectX::TexMetadata metaData{};
		DirectX::ScratchImage scratchImage{};
